...
import * as cc from "./old_consolecolor";
import { gzipSync } from "fflate";

//// throw new Error("test")
// Config
import * as config from './old_config';
// Alle Ressourceninformationen
import * as res from './old_resources';
import { resMapLayout } from "./old_resmap";
...
export function getSolarSpectralClassData(mass: number): { class: string, subclass: string, name: string, color: string, temp: number, mass_sol: number, lum_sol: number, lum: number, r_sol: number } {
    ...
    // Lum = L_sol * (r/r_sol)^2 * (T/T_sol)^4
    //// returnData.lum_sol = config.LUM_SOL_W * Math.pow((returnData.r_sol*config.R_SOL_KM)/config.R_SOL_KM, 2) * (returnData.temp/config.T_SOL, 4)
    // Lum_W = 4 * PI * r^2 * sigma * T^4
    returnData.lum
...
export function generateMoonSystemData(parentPlanetName: string, parentPlanetMass: number, maxMoons: number = 5): config.moonSystemDataDef[] {
    ...
    for (let i = 0; i < moonCount; i++) {
        ...
        moons.push({
            ...
            g: +g.toFixed(2),
            r: +r.toFixed(2), // Radius
            d: +d.toFixed(3), // Dichte
            OrbitalSpeed: +(OrbitalSpeed.toFixed(2)),
